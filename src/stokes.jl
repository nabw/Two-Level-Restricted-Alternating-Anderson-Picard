using Ferrite, FerriteGmsh, Gmsh, Tensors, LinearAlgebra, SparseArrays, TimerOutputs
import LinearAlgebra: ldiv!

# Lower triangular preconditioner
# It considers only
# [A  0]
# [B  S], whose inverse is
#
# [invA  0   ]
# [X     invS], where X = -invS * B * invA.
# We will use that the second row is
# yp = invS * xp - invS*B*invA * xu
#    = invS ( xp - B * invA * xu)
mutable struct blockP{matTu,matTp,matB,rangeT,vecY}
    Pu::matTu
    Pp::matTp
    B::matB
    dofsu::rangeT
...
function ldiv!(y::vecY, P::blockP, x::vecX) where {vecY,vecX} 
    ...
    # Second component
    #ldiv!(yu, P.Pu, xu) # yu = invA * xu
    #mul!(yp, P.B, yu) # yp = B * invA * xu
    #axpby!(1, xp, -1, yp) # yp = xp - B * invA * xu
    #ldiv!(P.Pp, yp) # yp = second row = X * xu + invS * xp
    ldiv!(yp, P.Pp, xp) # Diagonal version for 2nd component
...
function ldiv!(P::blockP, x::vecX) where {vecX}
    ldiv!(P.y, P, x)
    copy!(x, P.y)
end 
...
function assemble_system!(M, K, f, dh, cvu, cvp)
    ...
    for cell in CellIterator(dh)
        ...
        for qp in 1:getnquadpoints(cvu)
            ...
            for (i, I) in pairs(range_p), (j, J) in pairs(range_p)
                me[I, J] += -( ϕₚ[i] * ϕₚ[j] ) * dΩ
            end
            ## rhs
...
function assembleStokes(H)
    ## Grid
    h = H # approximate element size
    grid = setup_grid(h)
    ## Interpolations
    ...
    coupling_M = [true false; false true] # Diagonal blocks
    Mp = allocate_matrix(dh, ch; coupling=coupling_M)
    f = zeros(ndofs(dh))
    ## Assemble system
    assemble_system!(Mp, K, f, dh, cvu, cvp)
    ## Apply boundary conditions and solve
    apply!(K, f, ch)
    apply!(Mp, ch)

    # Build a block preconditioner
    range_u = dof_range(dh, :u)
    range_p = dof_range(dh, :p)
    ...
    ndofs_p = 0
    for i in 1:ncells
        ndofs_u += length(celldofs(dh, i)[range_u])
        ndofs_p += length(celldofs(dh, i)[range_p])
    end
        
    dofs_u = zeros(Int, ndofs_u)
    dofs_p = zeros(Int, ndofs_p)
    ju = 1
    jp = 1
    for c in 1:ncells
        for k in celldofs(dh, c)[range_u]
            dofs_u[ju] = k
            ju += 1
        end
        for k in celldofs(dh, c)[range_p]
            dofs_p[jp] = k
            jp += 1
        end
    end
